...
The dataset used consists of structured tabular healthcare data, which makes classical machine learning models more appropriate than deep learning approaches. Deep learning is typically more effective for unstructured data such as images or text, whereas this dataset contains categorical and numerical features.
...
Logistic Regression was selected as the final model after hyperparameter tuning using RandomizedSearchCV, as it provided the best balance between recall, stability, and interpretability.
...
Overfitting was addressed using cross-validation and evaluation on a separate test set. The consistency between validation and test results suggests that the model generalizes reasonably well.
